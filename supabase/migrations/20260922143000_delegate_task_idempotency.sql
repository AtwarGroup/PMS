...
-- Duplicate delivery of an already completed delegation must be a cheap no-op.
-- The check intentionally precedes optimistic revision validation: an old
-- browser may replay the successful request after receiving the Realtime event.
...
begin
  ...
  if v_task.assignee_id=p_target_id and v_task.delegated_by_id=v_uid then
