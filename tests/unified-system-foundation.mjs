...
assert.match(shell,/design-system\.css\?v=2\.5\.1/,'The shell must load the canonical design system');
...
for(const file of html){const source=readFileSync(file,'utf8'),versions=[...source.matchAll(/shell-components\.js\?v=([^"']+)/g)].map(match=>match[1]);for(const version of versions)assert.equal(version,'2.5.1',`${file} uses stale shell ${version}`);if(versions.length){assert.match(source,/ui-feedback\.js\?v=2\.5\.1/,`${file} must load unified feedback synchronously`);assert.match(source,/session-service\.js\?v=2\.5\.1/,`${file} must load session service synchronously`)}}
