...
for(const file of protectedPages){
  ...
  assert.match(source,/shell-components\.js\?v=2\.5\.1/,'Stale shell cache version: '+file);
  assert.match(source,/ui-feedback\.js\?v=2\.5\.1/,'Missing synchronous feedback: '+file);
  assert.match(source,/session-service\.js\?v=2\.5\.1/,'Missing synchronous session service: '+file);
  assert.match(source,/shared\.css\?v=2\.5\.1/,'Stale shared theme: '+file);
}
